...
function updateOverdueBadge() {
  const now = new Date();
  const overdueCount = allAccounts.filter(a => calcStatus(a, now) === 'overdue').length;
  try {
    const browserAPI = typeof browser !== 'undefined' ? browser : chrome;
    browserAPI.runtime.sendMessage({ type: 'updateOverdueCount', count: overdueCount });
  } catch (e) {
  ...
  return `
    <div class="account-row" data-id="${account.id}">
      ...
      <div class="row-actions">
        <button class="row-action-btn refresh-btn" data-id="${account.id}" data-action="refresh" title="Mark password as refreshed">&#x21bb;</button>
        <button class="row-action-btn" data-id="${account.id}" data-action="open" title="Open site">&#x2197;</button>
        <button class="row-action-btn delete-btn" data-id="${account.id}" data-action="delete" title="Delete">&#x2715;</button>
      </div>
    </div>
  `;
}
